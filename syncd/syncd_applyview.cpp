        ...
        _In_ const std::shared_ptr<SaiObj> &currentObj)
{
    ...
    for (const auto &it: currentObj->getAllAttributes())
    {
        ...
        auto sw = switches.begin()->second; // TODO

        auto defaultValueAttr = BestCandidateFinder::getSaiAttrFromDefaultValue(currentView, sw, *meta);
        ...
        _In_ bool performTransition)
{
    ...
    for (auto &at: temporaryObj->getAllAttributes())
    {
        ...
        if (SAI_HAS_FLAG_CREATE_AND_SET(meta->flags) && !conditional)
        {
            ...
            const auto defaultValueAttr = BestCandidateFinder::getSaiAttrFromDefaultValue(currentView, sw, *meta);
        ...
        _In_ const std::shared_ptr<SaiObj> &temporaryObj)
{
    ...
    auto bcf = std::make_shared<BestCandidateFinder>(currentView, temporaryView, sw);
